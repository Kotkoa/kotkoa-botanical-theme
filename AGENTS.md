...
### Tooling: image SEO skill and Admin API access

- **`.pi/skills/shopify-image-seo-cycle`** is a project skill implementing the gated, safe workflow for cleaning up **product** image filenames/alt text and deduplicating Printify variant mockups. It runs as three commands: `/image-1-audit <handle>` (read-only audit + proposed text), `/image-2-approve <handle>` (records human approval, no writes), `/image-3-apply <handle>` (applies the approved plan to one product, with before/after snapshots and verification). Non-negotiable rules baked into it: exactly one product handle per cycle, never `fileDelete` (dedup only detaches file references, never deletes files), a shared variant-media link is never treated as proof of visual duplication, and a live write is refused unless the plan status is exactly `approved-not-applied` and `scripts/validate_plan.py` passes. That validator enforces a product/variant plan shape, so it does not apply as-is to non-product images (e.g. homepage/collection card images) — those need the same phase discipline but a different, simpler plan shape (filename/alt only, no variant detach/append).
- **Admin API access** is already provisioned: a Dev Dashboard app named `Kotkoa Image SEO` uses the client-credentials grant with scopes `read_products`, `write_products`, `read_files`, `write_files`. Client ID/Secret live in the local `.env` (never committed, never pasted into chat); the skill's audit phase exchanges them for a short-lived Admin API token automatically. These scopes cover both product media and collection/file metadata, so no new app or scope is needed for other Admin GraphQL image work on this store (e.g. homepage collection-card images, collection featured images).
